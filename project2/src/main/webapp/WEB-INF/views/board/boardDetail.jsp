        ...
        <div class="board-header">
            ...
            <div class="board-info">
                ...
                <c:if test="${not empty board.imageList}">
                    ...
                    <!-- imageList의 이미지 개수가 start보다 클 경우 (썸네일 유무 관계 없이 다른 이미지가 있을 경우) -->
                    <c:if test="${fn:length(board.imageList) > start }">
                        ...
                        <div class="img-box">
                            <c:forEach var="i" begin="${start}" end="${fn:length(board.imageList) - 1}">
                                <div class="boardImg">
                                    <img
                                        src="${board.imageList[i].imagePath}${board.imageList[i].imageReName}">
...
<!-- JSP 내장 객체에 세팅 값을 JS에서 얻어가는 방법 두가지 -->
    <!--화면에 숨겨놓고 JS를 이용해 값을 얻어가는 방법 -->
    <!--<input type="hidden" name="memberNo" value="${loginMember.memberNo }">-->
    
    <!-- script 태그를 이용해 전역변수로 선언하는 방법 -->
    <script>
        // 로그인한 회원 번호 얻어오기
        // (참고) JSP 해석 순서 : EL -> JSTL -> JS
        // JS에서 EL/JSTL 사용 시 양쪽에 "" 또는 ''를 붙여줘야 함
