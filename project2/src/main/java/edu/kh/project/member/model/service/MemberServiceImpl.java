...
// @Service("이름") : bean 등록 시 지정된 이름으로 등록

@Service // Service 레이어, 비즈니스 로직을 가진 클래스임을 명시 + bean등록
public class MemberServiceImpl implements MemberService{
	...
	public Member login(Member inputMember) {
		...
		// bcrypt 이용 시 로그인 방법
		// 1. 이메일이 일치하는 회원 정보를 DB에서 조회, 반드시 비밀번호(MEMBER_PW) 포함해야한다.
		Member loginMember=dao.login(inputMember.getMemberEmail());
		
		// 2. 입력 받은 비밀번호 , 조회한 암호화된 비밀번호 비교 
		// -> BCrptPasswordEncoder.matches(평문, 암호문) 이용 -> 같으면 true, 아니면 false
		if(loginMember != null) { //아이디 정상 입력
			
			if(bcrypt.matches(inputMember.getMemberPw(), loginMember.getMemberPw())) {
				// 3-1. 비밀번호 일치하면 조회된 회원 정보 반환. 단, 비밀번호는 제거
