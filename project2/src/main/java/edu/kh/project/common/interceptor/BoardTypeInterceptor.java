...
//Dispatcher Servlet <-> 컨트롤러  사이의 요청/응답을 가로채 특정 코드를 수행(Spring)
...
public class BoardTypeInterceptor implements HandlerInterceptor {
	...
	//alt+shift+s 로 HandlerInterceptor 오버라이딩
	...
	//(보통 요청 처리중에 사용한 자원을 반환(close())하는데 많이 사용)
	
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		//appliaction scope에 게시팜 이름 목록(boardTypeList)이 있는지 확인해 없을 경우 DB에서 조회해 세팅
