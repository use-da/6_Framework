...
// IOC(제어의 역전)
//IoC(Inversion of Control)란 메서드나 객체의 호출작업을 개발자가 결정하는 것이 아니라, 외부에서 결정되는 것을 의미한다.
// -> bean등록 필요(@Component, @Controller, @Service, @Repository)
@Component
public class ImageDeleteScheduling {
	/*
	    ...
	    * L : 마지막. (월, 요일만 해당)
	    * 
	    * * 주의사항 - @Scheduled 어노테이션은 매개변수가 없는 메서드에만 적용 가능.
	    */
...
//		System.out.println("0초마다 출력");
		...
		//3. 둘을 비교해 Server 이미지 목록 중 DB에 없는 이미지를 삭제
		if(!fileList.isEmpty()) { //서버에 파일이 있다면
			...
			for(File file : fileList) {
				...
				if(dbList.indexOf(fileName)==-1) {
					...
					// trace, *debug, *info, *warn, *error, fatal *4개를 주로 사용
