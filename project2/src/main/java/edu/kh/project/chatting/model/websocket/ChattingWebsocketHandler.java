...
/* WebSocket
- 브라우저와 웹서버(session에 접속한 모든 객체 관리)간의 전이중통신을 지원하는 프로토콜이다
- HTML5버전부터 지원하는 기능이다.
...
public class ChattingWebsocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// 전달 받은 내용 확인
		logger.info("전달 받는 내용 : " + message.getPayload());

		// Jackson-databind에서 제공하는 ObjectMapper객체 사용
		...
		if (result > 0) {
			...
			// -> JSON으로 변환
			// -> 로그인한 회원 중 대상번호, 보낸사람번호가 일치하는 2명에게 웹소켓으로 메세지 전달
